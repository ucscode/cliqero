...
- The documented integration recipe supplies local `APP_URL`, database, media, and blog paths. The local outbox-worker startup failure was traced to a stale image expecting singular `config.pay_currency`; rebuilding the worker against the current source fixed startup. The full integration run still needs a separate clean pass before treating that suite as clean.
...
- The local PostgreSQL environment is resettable development state. The canonical PostgreSQL schema, including funding cancellation and evidence storage, is folded into `database/migrations/001_initial_schema.sql`; do not add numbered incremental migrations while destructive reset remains acceptable.
...
- NOWPayments sandbox acceptance uses the official create-payment `case: success` test procedure via the optional nested `sandbox: { case: success }` setting. The adapter never sends this field to the live API; the selected `pay_currency` comes from the configured `pay_currencies` allowlist, and the IPN callback must be publicly reachable. Sandbox cases do not involve real funds.
...
The current provider UX contract keeps bank preparation limited to account selection and the quote; persisted status shows the immutable account snapshot, provider display name, generic funding reference, and explicit bank narration instruction. Bank fields are opaque and may opt into `copyable` presentation metadata; visual prominence belongs to the status layout. Paystack collection capability is provider-owned (`config.currencies` plus an in-list `default_currency`), with country mapping used only as a preference. Shared exchange rates are PostgreSQL-backed and fresh for 24 hours by default; only the exact persisted rate is used for conversion, while the UI renders a two-decimal rate without the internal source. Paystack hosted continuation uses same-tab navigation. NOWPayments status shows exact payment instructions, automatic detection, refresh, and authoritative expiry feedback; minimum diagnostics carry minor units so `1915` USD is rendered as `$19.15`, including legacy rows matching the known structured diagnostic format.

Payment providers are modular funding modules. The generic funding entry at `/dashboard/wallet/fund` selects canonical amount/provider only and does not create funding or initialize a provider. Provider preparation uses `/dashboard/wallet/fund/{provider}?amount=...`; explicit provider-page Proceed then enters the normal persisted funding workflow. `/dashboard/wallet/fund?funding={id}` remains the canonical resumable status/payment URL. Awaiting payments may continue with their persisted provider authorization, while initialization and verification states show progress without creating another provider transaction. Paystack browser callbacks redirect to this status view using `APP_URL` after delegating to the normal funding verification processor; they do not manufacture `verification_pending` or confirm from query parameters. Ambiguous Paystack verification failures are bounded by persisted failure accounting before reconciliation. `0.0.0.0` remains only a server bind address. Genuine signed Paystack webhook delivery remains unproven.
...
The canonical PostgreSQL baseline includes the runtime funding state union and `funding_state_valid` constraint for `cancelled`, plus the funding-evidence storage schema. Cancellation preserves the funding row and creates no wallet credit.
