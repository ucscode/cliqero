...
NOWPayments returns a payment address/amount/currency and optional asset/network metadata. Bank transfer supports multiple configured receiving accounts, each with independent country/currency filters and ordered opaque `key`/`label`/`value` display fields. Cliqero does not interpret banking terminology. Initialization selects an eligible account and persists its stable account ID plus rendered instructions in the funding record, so later configuration changes do not erase the original payment facts. It reports `awaiting_manual_confirmation`; no customer submission alone confirms funding. Finance-authorized operators can explicitly confirm a persisted `bank_transfer` funding record through `POST /api/operator/funding/{fundingId}/confirm-bank-transfer`; the state change and audit fact are committed together, and wallet credit remains a separate downstream processor.

The UI funding screen should consume `/api/wallet/funding-methods`, which filters enabled providers by authenticated account country and requested collection currency. The development provider is registered only in development/test runtimes and must never be exposed by a production registry. It must not duplicate provider eligibility rules. Checkout also requests a server-calculated quote before checkout creation so the initial screen can show the available wallet balance and shortfall.
