...
export function ReferralsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [directPage, hierarchy, uplinePage] = await Promise.all([
        apiFetch<ReferralPage>("/api/referrals/direct?limit=50"),
        apiFetch<HierarchyTree>("/api/hierarchy/tree"),
    ...
    <section className="referral-panel" aria-labelledby="referrals-heading">
      <div className="panel-heading">
        <div>
          ...
          <h2 id="referrals-heading">Your referral context</h2>
          <p className="panel-intro">
            Explore direct referrals and the bounded hierarchy window provided for your account.
            Financial information for other accounts is never shown here.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="referral-grid">
                ...
                {direct.accounts.map((id) => {
                  ...
                  return (
                    <li key={id}>
                      ...
                      <span>
                        <strong>{node?.handle ?? "Cliqero account"}</strong>
                        <small>{id}</small>
                      </span>
                    </li>
                  );
                  ...
                  return (
                    <li key={upline.accountId}>
                      ...
                      <span>
                        <strong>{node?.handle ?? "Cliqero account"}</strong>
                        <small>
                          Level {upline.depth} · {upline.accountId}
                        </small>
                      </span>
                    </li>
                  );
          ...
          <Card className="referral-section-card referral-downline-card">
            <div className="card-kicker">
              <h3>Downline window</h3>
              <Badge tone="accent">{tree ? `${tree.windowDepth} levels` : "—"}</Badge>
            </div>
            <p className="panel-note">
              Showing up to {tree?.childLimit ?? "the configured"} children per branch in this
              window. Rebase through the hierarchy API to explore deeper generations.
            </p>
            {generations.length ? (
              ...
              <EmptyState
                ...
                description="Descendant accounts will appear here when your network grows."
              />
            )}
          </Card>
        </div>
