...
export class PaymentInitializationProcessor {
  ...
  async process(paymentId: string): Promise<void> {
    const payment = await this.payments.findById(paymentId, { forUpdate: false });
    if (!payment) return;
    ...
    try {
      ...
      await this.uow.transaction(async () => {
        ...
        if (!locked) return;
        locked.state =
          diagnostic.kind === "ambiguous" ? "reconciliation_pending" : "initialization_blocked";
        await this.payments.save(locked);
      });
    }
  }
