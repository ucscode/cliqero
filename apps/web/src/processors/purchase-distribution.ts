...
export class PurchaseDistributionProcessor {
  ...
  async process(input: {
    ...
    return this.uow.transaction(async () => {
      ...
      if (purchase.checkoutId && this.yamlCommissionPolicy)
        return this.processWalletDistribution(purchase, gross, input);
      const [commissionPolicy, financialPolicy] = await Promise.all([
      ...
      const referralRate = referralFacts.reduce(
        (sum, fact) => sum + fact.configuredRateBasisPoints,
        0,
      );
      const totalPolicyRate = referralRate + financialPolicy.platformRateBasisPoints;
      if (totalPolicyRate > 10000)
        throw new Error("Distribution policy exceeds gross purchase amount");
      const sellerRate = 10000 - totalPolicyRate;
      let sellerMinor = (gross.minorAmount * BigInt(sellerRate)) / 10000n;
      let platformMinor =
        (gross.minorAmount * BigInt(financialPolicy.platformRateBasisPoints)) / 10000n;
      const referralMinor = referralFacts.reduce(
        (sum, fact) => sum + fact.calculatedAmount.minorAmount,
        0n,
      );
      const residual = gross.minorAmount - sellerMinor - platformMinor - referralMinor;
      if (financialPolicy.remainderRecipient === "seller") sellerMinor += residual;
      else platformMinor += residual;
      ...
      const add = (
        ...
        entries.push({
          ...
          basis,
          referralLevel: level,
          balanceState: financialPolicy.initialBalanceState,
          maturityAt:
            financialPolicy.initialBalanceState === "pending"
      ...
      for (const fact of referralFacts)
        add(
      ...
      const policySnapshot = {
        sellerRateBasisPoints: sellerRate,
        platformRateBasisPoints: financialPolicy.platformRateBasisPoints,
        referralRatesBasisPoints: referralFacts.map((f) => ({
          level: f.level,
          rate: f.configuredRateBasisPoints,
        })),
        remainderRecipient: financialPolicy.remainderRecipient,
        remainderMinor: residual.toString(),
  ...
  private async processWalletDistribution(
    purchase: Purchase,
    gross: Money,
    input: { purchaseId: string; correlationId: string },
  ) {
    const policy = await this.yamlCommissionPolicy!.getActive();
    const facts = await this.commissions.calculate(purchase, policy);
    const userTotal = facts.reduce((sum, fact) => sum + fact.calculatedAmount.minorAmount, 0n);
    const platform = gross.minorAmount - userTotal;
    if (platform < 0n) throw new Error("Commission distribution exceeds purchase gross");
    const distributionId = newId();
    const entries: LedgerEntryDraft[] = facts
      .filter((fact) => fact.calculatedAmount.minorAmount > 0n)
      .map((fact) => ({
        id: newId(),
        distributionId,
        accountId: fact.recipientAccountId,
        purchaseId: purchase.id,
        entryType: "purchase-earnings" as const,
        direction: "credit" as const,
        amount: fact.calculatedAmount,
        idempotencyKey: `purchase-distribution:${purchase.id}:referral:${fact.level}:${fact.recipientAccountId}`,
        correlationId: input.correlationId,
        recipientRole: "referral" as const,
        basis: fact.basis,
        referralLevel: fact.level,
        balanceState: "available" as const,
      }));
    const configured = policy.percentages.map((percentage, index) => ({
      level: index + 1,
      percentage,
      recipient: facts.find((fact) => fact.level === index + 1)?.recipientAccountId ?? null,
      amountMinor: (
        facts.find((fact) => fact.level === index + 1)?.calculatedAmount.minorAmount ?? 0n
      ).toString(),
    }));
    const policySnapshot = {
      version: "yaml",
      levels: configured,
      allocatedPercentage: policy.percentages.reduce((sum, value) => sum + value, 0),
      platformRemainderMinor: platform.toString(),
      grossMinor: gross.minorAmount.toString(),
      currency: gross.currency,
    };
    await this.ledger.createDistribution({
    ...
    if (entries.length) await this.ledger.append(entries);
    await this.outbox.append([
      {
        id: newId(),
        name: "purchase.distribution.completed",
    ...
    return (await this.ledger.findDistributionByPurchaseId(purchase.id))!;
  }
}
