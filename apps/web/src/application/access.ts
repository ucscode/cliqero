...
export class BuyerAccessService {
  ...
  async handoff(buyer: Account, listingId: string, idempotencyKey?: string): Promise<URL> {
    ...
    const issued = await this.uow.transaction(() =>
      this.access.issue(buyer.id, listing.id, idempotencyKey),
    );
    const destination = new URL(listing.destination);
    destination.searchParams.set("source", issued.source);
    return destination;
  }
  async handoffPurchase(buyer: Account, purchaseId: string, idempotencyKey?: string): Promise<URL> {
    ...
    const entitlement = await this.entitlements?.findByPurchaseId(purchase.id);
    if (!entitlement?.isActive) throw new Error("Active entitlement not found");
    const listing = await this.listings.findById(purchase.terms.listingId);
