...
import type { PaymentRecord,PaymentRepository,PaymentProviderRegistry } from "@/modules/payment/payment";
...
export class CheckoutService {
  constructor(private listings:ListingRepository,private payments:PaymentRepository,private purchases:PurchaseRepository,
    ...
    private attribution:PurchaseAttributionResolver,private uow:UnitOfWork,private accounts?:AccountReader,private exchangeRates?:ExchangeRateService) {}
  async initiate(input:{buyerId:Id;buyerEmail:string;listingId:Id;providerName:string;idempotencyKey:string;attributionSource?:string;collectionCurrency?:string}):Promise<{paymentId:Id;purchaseId:Id;provider:string;providerReference:string;authorizationUrl?:string;accessCode?:string}> {
    const existing=await this.payments.findByIdempotencyKey(input.idempotencyKey);
    if(existing) {const existingPurchase=await this.purchases.findByIdempotencyKey(input.idempotencyKey);if(!existingPurchase)throw new Error("Checkout purchase is missing");return {paymentId:existing.id,purchaseId:existingPurchase.id,
    ...
    const provider=this.providers.get(input.providerName,{country:buyer?.country??null,currency:collectionCurrency});
    const prepared=await this.uow.transaction(async()=>{
      ...
      if(!claimed) {
        ...
        if(prior&&priorPurchase)return {existing:true as const,result:{paymentId:prior.id,purchaseId:priorPurchase.id,provider:prior.providerName,providerReference:prior.providerReference,
      ...
      const quote=collectionCurrency===listing.price.currency?undefined:await this.exchangeRates?.quote(listing.price.currency,collectionCurrency);
      if(collectionCurrency!==listing.price.currency&&!quote) throw new Error("Exchange rate is unavailable");
      const collectionAmount=quote?new ExactCurrencyConverter().convert(listing.price,quote):listing.price;
      const providerReference=provider.referenceFor?.({paymentId,idempotencyKey:input.idempotencyKey})??`pay-${paymentId}`;
      const payment={id:paymentId,providerName:provider.name,providerReference,buyerId:input.buyerId,
        listingId:listing.id,amount:collectionAmount,collectionAmount,canonicalAmount:Money.of(listing.price.minorAmount,"USD"),idempotencyKey:input.idempotencyKey,
        state:"initialization_pending" as const,conversionSnapshot:quote?{fromCurrency:quote.fromCurrency,toCurrency:quote.toCurrency,rate:quote.rate,source:quote.source,sourceDate:quote.sourceDate,observedAt:quote.observedAt}:undefined,
    ...
    await this.idempotency.complete("checkout",input.idempotencyKey,prepared.purchase.id,{paymentId:prepared.payment.id,purchaseId:prepared.purchase.id});
    return {paymentId:prepared.payment.id,purchaseId:prepared.purchase.id,provider:provider.name,providerReference:prepared.payment.providerReference};
  }
}
...
export class PaymentCompletionService {
  ...
  async complete(input:{paymentId:Id;correlationId:Id}):Promise<Entitlement> {
    ...
    const verified=await this.providers.get(payment.providerName).verify({reference:payment.providerReference,expectedAmount:collectionAmount});
    ...
    if(!verified.amount.equals(collectionAmount))throw new Error("Payment amount or currency mismatch");
