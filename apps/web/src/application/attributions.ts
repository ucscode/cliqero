...
export class ReferralAttributionService implements PurchaseAttributionResolver {
  ...
  async createLink(referrerAccountId: string, listingId: string) {
    ...
    return this.attributions.createOrGetLink({
      id: newId(),
  ...
  async visit(code: string): Promise<{ listingId: string; source: string } | null> {
    const link = await this.attributions.findActiveLinkByCode(code);
    if (!link) return null;
    const listing = await this.listings.findById(link.listingId);
  ...
  listLinks(accountId: string) {
    return this.attributions.listLinks(accountId);
  }
  async getLink(accountId: string, id: string) {
    const link = await this.attributions.findLinkById(id);
    if (!link || link.referrerAccountId !== accountId) throw new Error("Referral link not found");
    return link;
  }
  async revokeLink(accountId: string, id: string) {
    await this.getLink(accountId, id);
    await this.attributions.revokeLink(id, accountId);
    return this.getLink(accountId, id);
  }
