...
const bodySchema=z.object({listing_id:z.uuid(),provider:z.string().default("development")});
export async function POST(request:Request) {
  ...
  try { const body=bodySchema.parse(await request.json());
    return Response.json(await getContainer().checkout.initiate({buyerId:account.id,buyerEmail:account.email,listingId:body.listing_id,
      providerName:body.provider,idempotencyKey,attributionSource:referralAttributionSource(request)}),{status:201});
