...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return Response.json(
      view(await getContainer().referralAttribution.getLink(a.id, (await params).id), request),
    );
  } catch (error) {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return Response.json(
      view(await getContainer().referralAttribution.revokeLink(a.id, (await params).id), request),
    );
  } catch (error) {
    return apiError(error);
  }
