...
export async function POST(request:Request) {
  try {
    ...
    const upstream=await auth.auth.handler(new Request(upstreamUrl,{method:"POST",headers:forwardedHeaders,body:JSON.stringify(body)}));
    ...
    const response=Response.json({account:{id:account.id,email:account.email,handle:account.handle},token:payload.token});
