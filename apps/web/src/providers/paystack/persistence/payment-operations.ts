...
export class PostgresPaymentOperationsRepository {
  ...
  async recordFundingSuccess(input:{fundingId:string;provider:string;operation:string;providerMessage?:string}){await this.sql.query(`insert into payment_capability.provider_operations(id,funding_id,provider,operation,outcome,provider_message) values($1,$2,$3,$4,'succeeded',$5)`,[newId(),input.fundingId,input.provider,input.operation,input.providerMessage??null]);}
  async recordFundingFailure(input:{fundingId:string;provider:string;operation:string;error:{httpStatus?:number;providerStatus?:boolean;providerMessage:string;providerCode?:string;kind:string}}){await this.sql.query(`insert into payment_capability.provider_operations(id,funding_id,provider,operation,outcome,http_status,provider_status,provider_message,provider_code,failure_kind) values($1,$2,$3,$4,'failed',$5,$6,$7,$8,$9)`,[newId(),input.fundingId,input.provider,input.operation,input.error.httpStatus??null,input.error.providerStatus??null,input.error.providerMessage.slice(0,1000),input.error.providerCode??null,input.error.kind]);}
