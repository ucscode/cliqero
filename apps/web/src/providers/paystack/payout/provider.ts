...
export class PaystackPayoutProvider implements PayoutProvider {
  ...
  async submitPayout(input: {
    ...
    if (!recipient) {
      const data = await this.request<Recipient>("/transferrecipient", {
        method: "POST",
        body: JSON.stringify({
          type: "nuban",
    ...
    const data = await this.request<Transfer>("/transfer", {
      method: "POST",
      body: JSON.stringify({
        source: "balance",
        amount: input.withdrawal.amount.minorAmount.toString(),
        recipient,
  ...
  private async request<T>(path: string, init: RequestInit): Promise<T> {
    ...
    try {
      ...
      throw Object.assign(
        new Error(error instanceof Error ? error.message : "Paystack request failed"),
        { unknownOutcome: path === "/transfer" },
      );
    }
    ...
    try {
      ...
      throw new Error(`Paystack returned invalid JSON (${response.status})`);
    }
    if (!response.ok || !envelope.status || envelope.data === undefined) {
      const error = Object.assign(
        new Error(
          `Paystack request failed (${response.status}): ${envelope.message || "Unknown error"}`,
        ),
        {
          failureCategory:
