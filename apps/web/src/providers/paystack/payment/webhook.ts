...
interface PaystackRefundProcessed {
...
export class PaystackWebhookIngress {
  ...
  async ingest(
  ...
  ): Promise<{ authenticated: boolean; duplicate?: boolean; accepted?: boolean }> {
    ...
    try {
      ...
      throw new Error("Paystack webhook JSON is invalid");
    }
    if (isRefundProcessed(payload)) {
      const eventKey = `refund.processed:${payload.data.id}`;
      return this.uow.transaction(async () => {
        const recorded = await this.events.record({
          ...
          amountMinor: String(payload.data.amount),
        ...
        await this.outbox.append([
          {
            id: newId(),
            name: "payment.paystack.refund-processed",
            aggregateId: recorded.record.id,
            correlationId: recorded.record.id,
            occurredAt: new Date(),
            payload: { providerEventId: recorded.record.id },
          },
        ]);
    ...
    if (!isChargeSuccess(payload)) {
      const eventType =
        isObject(payload) && typeof payload.event === "string" ? payload.event : "unknown";
      const recorded = await this.events.record({
        id: newId(),
        providerName: "paystack",
        eventKey: `${eventType}:${createHash("sha256").update(rawBody).digest("hex")}`,
        eventType,
      ...
      if (recorded.created)
        await this.events.markIgnored(
          recorded.record.id,
          "Authenticated Paystack event has no current commerce consequence",
        );
      return { authenticated: true, accepted: false };
    }
    const eventKey = `charge.success:${payload.data.id}`;
    const id = newId();
    return this.uow.transaction(async () => {
      const recorded = await this.events.record({
        id,
      ...
      await this.outbox.append([
        {
          id: newId(),
          name: "payment.paystack.charge-succeeded",
          aggregateId: recorded.record.id,
          correlationId: recorded.record.id,
          occurredAt: new Date(),
          payload: { providerEventId: recorded.record.id },
...
function isChargeSuccess(value: unknown): value is PaystackChargeSuccess {
  ...
  return (
    ...
    Number.isSafeInteger(value.data.amount) &&
    Number(value.data.amount) >= 0 &&
    typeof value.data.currency === "string" &&
    typeof value.data.status === "string"
  );
}
function isRefundProcessed(value: unknown): value is PaystackRefundProcessed {
  if (!isObject(value) || value.event !== "refund.processed" || !isObject(value.data)) return false;
  return (
    Number.isSafeInteger(value.data.id) &&
    typeof value.data.transaction_reference === "string" &&
    value.data.transaction_reference.length > 0 &&
    Number.isSafeInteger(value.data.amount) &&
    Number(value.data.amount) > 0 &&
    typeof value.data.currency === "string" &&
