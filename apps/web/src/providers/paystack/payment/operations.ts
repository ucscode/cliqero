...
export class PaymentReconciliationService {
  constructor(private readonly payments:PostgresPaymentRepository,private readonly verification:PaymentVerificationProcessor,
  ...
  async reconcile(input:{actorId:string;paymentId:string;idempotencyKey:string;correlationId:string}):Promise<ReconciliationAttempt>{
    ...
    try{payment.state="verification_pending";await this.payments.save(payment);await this.verification.process(payment.id);const verified=await this.payments.findById(payment.id);if(verified?.state!=="verified")throw new Error("Payment verification mismatch");
      await this.operations.finish(begun.attempt.id,"completed",{paymentState:"verified"});return {...begun.attempt,state:"completed",result:{paymentState:"verified"}};
