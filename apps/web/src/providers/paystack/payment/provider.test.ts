...
describe("PaystackProvider",()=>{
  it("maps checkout initialization using server-owned subunits, email, currency, and reference",async()=>{
    ...
    const [url,request]=http.mock.calls[0];const body=JSON.parse(String(request?.body));
  ...
  it("maps authoritative verification facts",async()=>{
    const http=vi.fn(async()=>Response.json({status:true,message:"Verification successful",data:{
      id:4099260516,status:"success",reference:"reference-1",amount:40333,currency:"NGN",fees:123}}));
    const result=await new PaystackProvider(config,http).verify({reference:"reference-1",expectedAmount:Money.of(40333n,"NGN")});
    expect(result).toMatchObject({verified:true,status:"success",reference:"reference-1",providerTransactionId:"4099260516"});
  ...
  it("validates the exact raw webhook payload with HMAC-SHA512",()=>{
    const provider=new PaystackProvider(config,vi.fn());const raw=Buffer.from('{"event":"charge.success","data":{"id":1}}');
