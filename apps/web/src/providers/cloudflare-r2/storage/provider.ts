...
export class CloudflareR2ObjectStorageProvider implements ObjectStorageProvider {
  ...
  private async request(method: string, key: string, body: Uint8Array, contentType: string) {
    const url = new URL(
      `${this.endpoint.replace(/\/$/, "")}/${encodeURIComponent(this.bucket)}/${key.split("/").map(encodeURIComponent).join("/")}`,
    );
    const now = new Date(),
    ...
    const signingKey = hmac(
      hmac(hmac(hmac(toBytes(`AWS4${this.secretAccessKey}`), day), "auto"), "s3"),
      "aws4_request",
    );
