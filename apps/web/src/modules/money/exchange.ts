...
export class ExactCurrencyConverter implements CurrencyConverter {
  constructor(private readonly units:{from:number;to:number}={from:2,to:2}) {
    if(!Number.isInteger(units.from)||!Number.isInteger(units.to)||units.from<0||units.to<0) throw new DomainInvariantError("Currency minor-unit scales must be non-negative integers");
  }
  convert(amount:Money,quote:ExchangeRateQuote):Money {
    if(amount.currency!==quote.fromCurrency.toUpperCase()) throw new DomainInvariantError("Exchange quote source currency does not match amount");
...
function parseDecimal(value:string):{numerator:bigint;denominator:bigint} {
  ...
  if(!/^(?:0|[1-9]\d*)(?:\.\d+)?$/.test(normalized)||normalized==="0") throw new DomainInvariantError("Exchange rate must be a positive decimal string");
