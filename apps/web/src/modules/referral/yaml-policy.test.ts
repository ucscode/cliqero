...
describe("YAML commission policy",()=>{
  ...
  it("does not impose an artificial hierarchy depth ceiling",()=>{const levels=Object.fromEntries(Array.from({length:33},(_,index)=>[index+1,1]));expect(commissionPolicyFromYaml({distribution:{commission:{levels}}}).maximumRewardedDepth).toBe(33);});
