...
export interface FinancialDistributionPolicy {
  platformAccountId: string;
  platformRateBasisPoints: number;
  remainderRecipient: "seller" | "platform";
  initialBalanceState: "pending" | "available";
  settlementDelaySeconds: number;
}
export interface FinancialDistributionPolicyRepository {
...
export interface LedgerRepository {
  ...
  summarizeAccount(
    accountId: string,
  ): Promise<readonly { currency: string; balanceState: string; amountMinor: bigint }[]>;
}
