...
export class AuthenticationService {
  ...
  async register(input:{email:string;handle:string;password:string}):Promise<Account> {
    if (input.password.length < 12) throw new Error("Password must contain at least 12 characters");
    const account = new Account(newId(), input.email.trim().toLowerCase(), input.handle.trim().toLowerCase());
    const salt=randomBytes(16);
    const hash=Buffer.from(await scrypt(input.password,salt,64) as Buffer);
    await this.sql.query(
      `insert into identity_capability.accounts (id,email,handle,password_salt,password_hash)
       values ($1,$2,$3,$4,$5)`,[account.id,account.email,account.handle,salt,hash]);
    return account;
  }
  ...
  async login(email:string,password:string):Promise<{account:Account;token:string}> {
    const row=(await this.sql.query<CredentialRow>(
      `select id,email,handle,password_salt,password_hash from identity_capability.accounts where email=$1`,[email.trim().toLowerCase()])).rows[0];
    ...
    return {account:new Account(row.id,row.email,row.handle),token};
  }

  async authenticate(token:string):Promise<Account|null> {
    if (!token || token.length>200) return null;
    const row=(await this.sql.query<CredentialRow>(
      `select account.id,account.email,account.handle,account.password_salt,account.password_hash
    ...
    return new Account(row.id,row.email,row.handle);
  }
}
