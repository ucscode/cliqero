...
export class Listing {
  ...
  static create(input: {
    ...
    destination: string; metadata?: ListingMetadata;
  }): Listing {
    ...
    return new Listing(input.id, input.sellerId, title, input.description.trim(), input.price, destination, input.metadata ?? {}, "draft");
  }

  static restore(input: {
    id: Id; sellerId: Id; title: string; description: string; price: Money;
    destination: string; metadata: ListingMetadata; state: ListingState;
  }): Listing {
    return new Listing(input.id, input.sellerId, input.title, input.description, input.price, new URL(input.destination), input.metadata, input.state);
  }
