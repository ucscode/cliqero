...
export class PostgresListingMediaRepository implements ListingMediaRepository {
  ...
  async findById(id: string) {
    const row = (
      await this.sql.query<any>(
        `select m.*,m.uuid as id,(select uuid from listing_capability.listings where id=m.listing_id) as listing_id from listing_capability.media m where m.uuid=$1`,
        [id],
      )
    ).rows[0];
  ...
  async findByStorageIdentity(provider: string, container: string, key: string) {
    const row = (
      await this.sql.query<any>(
        `select m.*,m.uuid as id,(select uuid from listing_capability.listings where id=m.listing_id) as listing_id from listing_capability.media m where storage_provider=$1 and storage_container=$2 and object_key=$3`,
  ...
  async listByListings(ids: readonly string[]) {
    ...
    for (const row of (
      await this.sql.query<any>(
        `select m.*,m.uuid as id,l.uuid as listing_id from listing_capability.media m join listing_capability.listings l on l.id=m.listing_id where l.uuid=any($1::uuid[]) and m.state='active' order by m.listing_id,m.position,m.created_at,m.id`,
        [ids],
      )
    ).rows)
  ...
  async reorderActive(listingId: string, orderedIds: readonly string[]) {
    await this.sql.query(
      `update listing_capability.media set position=position+1000000 where listing_id=(select id from listing_capability.listings where uuid=$1) and state='active'`,
      [listingId],
    );
    for (let position = 0; position < orderedIds.length; position++)
      await this.sql.query(
        `update listing_capability.media set position=$1,updated_at=now() where uuid=$2 and listing_id=(select id from listing_capability.listings where uuid=$3) and state='active'`,
  ...
  async claimDeletionWork(limit = 50, leaseMs = 5 * 60_000) {
    const rows = (
      ...
      from candidates where media.id=candidates.id returning media.*,(select uuid from listing_capability.listings where id=media.listing_id) as listing_uuid`,
        [limit, leaseMs],
      )
    ).rows;
    return rows.map(map);
