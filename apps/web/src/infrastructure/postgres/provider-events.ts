...
export class PostgresProviderEventRepository {
  ...
  async record(
    ...
    const existing = await this.findByIdentity(event.providerName, event.eventKey);
  ...
  async markProcessed(id: string): Promise<void> {
    await this.sql.query(
      `update payment_capability.provider_events set state='processed',processed_at=now(),last_error=null where id=$1`,
      [id],
    );
  }
  async markIgnored(id: string, reason: string): Promise<void> {
    await this.sql.query(
      `update payment_capability.provider_events set state='ignored',processed_at=now(),last_error=$2 where id=$1`,
      [id, reason.slice(0, 4000)],
    );
  }
  async markRejected(id: string, reason: string): Promise<void> {
    await this.sql.query(
      `update payment_capability.provider_events set state='rejected',processed_at=now(),last_error=$2 where id=$1`,
      [id, reason.slice(0, 4000)],
    );
  }
  private async findByIdentity(provider: string, key: string): Promise<ProviderEventRecord | null> {
    const row = (
      await this.sql.query<ProviderEventRow>(
        `select id,provider_name,event_key,event_type,provider_reference,amount_minor,currency,payload,state,last_error
       from payment_capability.provider_events where provider_name=$1 and event_key=$2`,
        [provider, key],
      )
    ).rows[0];
    return row ? this.map(row) : null;
  }
  private map(row: ProviderEventRow): ProviderEventRecord {
    return {
      id: row.id,
