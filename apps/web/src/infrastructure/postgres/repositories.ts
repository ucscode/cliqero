...
export class PostgresListingRepository implements ListingRepository {
  ...
  async findAnyByExternalKey(key:string){const row=(await this.sql.query<ListingRow>(`select id,seller_id,title,description,price_minor,price_currency,destination_url,metadata,state,external_key from listing_capability.listings where external_key=$1`,[key])).rows[0];return row?this.restore(row):null;}
