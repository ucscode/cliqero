...
export class PostgresIdempotencyRepository {
  ...
  async begin(scope: string, key: string): Promise<boolean> {
    const result = await this.sql.query(
      `insert into kernel.idempotency_records (scope, idempotency_key, state)
       values ($1, $2, 'processing') on conflict (scope, idempotency_key) do nothing`,
      [scope, key],
    );
  ...
  async complete(
    ...
    await this.sql.query(
      ...
      [scope, key, resultReference, JSON.stringify(response)],
    );
  }
  async fail(scope: string, key: string, error: string): Promise<void> {
    await this.sql.query(
      `update kernel.idempotency_records set state='failed',response=$3::jsonb,updated_at=now() where scope=$1 and idempotency_key=$2`,
      [scope, key, JSON.stringify({ error: error.slice(0, 4000) })],
    );
  }
  ...
  async findCompleted(scope: string, key: string): Promise<IdempotencyResult | null> {
    const result = await this.sql.query<{ result_reference: string | null; response: unknown }>(
      `select result_reference, response from kernel.idempotency_records
       where scope = $1 and idempotency_key = $2 and state = 'completed'`,
      [scope, key],
    );
