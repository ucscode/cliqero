...
suite("persisted commerce and access vertical path", () => {
  ...
  it("completes payment concurrently exactly once with entitlement and transactional outbox", async () => {
    ...
    const events = await app.database.query<{ event_name: string }>(
  ...
  it("rolls an outbox record back with its surrounding domain transaction", async () => {
    ...
    expect(
      (await app.database.query(`select id from kernel.outbox_events where id=$1`, [eventId]))
        .rowCount,
    ).toBe(0);
  });
  ...
  it("hands an entitled buyer an opaque source while persisting only its hash", async () => {
    ...
    expect(source).not.toContain(buyer.id);
    expect(source).not.toContain(listing.id);
    const stored = (
      await app.database.query<{ token_hash: Buffer }>(
        `select token_hash from access_capability.access_grants`,
      )
    ).rows[0].token_hash;
    expect(stored).toHaveLength(32);
    expect(stored.toString("utf8")).not.toBe(source);
  });
  ...
  it("requires an active entitlement and independently authenticated, listing-scoped integration", async () => {
    ...
    const destination = await app.buyerAccess.handoff(buyer, listing.id);
    const source = destination.searchParams.get("source")!;
    const validCredential = await app.database.transaction(() =>
      app.integrations.create(seller.id, "destination", listing.id),
    );
    ...
    await expect(app.access.verify(source, validIntegration!)).resolves.toMatchObject({
      authorized: true,
      buyerId: buyer.id,
      listingId: listing.id,
    });
    ...
    const otherCredential = await app.database.transaction(() =>
      app.integrations.create(seller.id, "other", otherListing.id),
    );
    const otherIntegration = await app.integrations.authenticate(otherCredential.credential);
    await expect(app.access.verify(source, otherIntegration!)).resolves.toEqual({
      authorized: false,
    });
    await expect(app.access.verify("purchase-id-or-malformed", validIntegration!)).resolves.toEqual(
      { authorized: false },
    );

    entitlement.revoke();
    await app.entitlements.save(entitlement);
    await expect(app.access.verify(source, validIntegration!)).resolves.toEqual({
      authorized: false,
    });
    await expect(app.buyerAccess.handoff(buyer, listing.id)).rejects.toThrow(
