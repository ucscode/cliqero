...
suite("provider-neutral payout execution", () => {
  const app = createContainer(databaseUrl!);
  beforeEach(async () => {
    await app.database.query(
      `truncate table payout_capability.attempts,payout_capability.executions,withdrawal_capability.withdrawals,ledger_capability.withdrawal_reservation_events,ledger_capability.withdrawal_reservations,ledger_capability.entry_settlements,ledger_capability.entries,ledger_capability.purchase_distributions,payment_capability.reconciliation_attempts,payment_capability.provider_events,access_capability.access_grants,entitlement_capability.entitlements,purchase_capability.purchases,payment_capability.payments,listing_capability.listings,identity_capability.account_capabilities,identity_capability.sessions,identity_capability.accounts,kernel.outbox_events,kernel.idempotency_records restart identity cascade`,
    );
    await app.database.query(
    ...
    await app.database.query(
      `update withdrawal_capability.policy set minimum_amount_minor=1,enabled=true`,
    );
  });
  ...
  async function setup(destinationReference = "normal") {
    ...
    await app.database.query(`update withdrawal_capability.policy set minimum_amount_minor=1`);
  ...
  it("submits only approved withdrawals and completes exactly once on verified success", async () => {
    const { seller, withdrawal } = await setup();
    await expect(app.payoutExecution.execute(withdrawal.id, newId())).resolves.toMatchObject({
      kind: "succeeded",
    });
    await expect(app.payoutExecution.execute(withdrawal.id, newId())).resolves.toMatchObject({
      state: "succeeded",
  ...
  it("preserves reservation on retryable and permanent failures", async () => {
    const retry = await setup("dev:retry");
    ...
    const permanent = await setup("dev:permanent");
    await expect(
      app.payoutExecution.execute(permanent.withdrawal.id, newId()),
    ).resolves.toMatchObject({ kind: "failed", category: "permanent_validation" });
    expect((await app.fundsReservation.summarize(permanent.seller.id))[0].reservedMinor).toBe(
      5000n,
    );
  });
  it("does not blindly retry unknown outcomes and reconciles verified success", async () => {
    const { seller, withdrawal } = await setup("dev:unknown");
    await expect(app.payoutExecution.execute(withdrawal.id, newId())).resolves.toMatchObject({
      kind: "unknown",
    });
    expect((await app.fundsReservation.summarize(seller.id))[0].reservedMinor).toBe(5000n);
    await expect(app.payoutExecution.execute(withdrawal.id, newId())).resolves.toMatchObject({
      state: "unknown",
    });
    await expect(app.payoutExecution.reconcile(withdrawal.id, newId())).resolves.toMatchObject({
      kind: "succeeded",
    });
    expect((await app.withdrawalRepository.findById(withdrawal.id))?.state).toBe("completed");
    expect(await app.payoutExecution.attempts(withdrawal.id)).toHaveLength(1);
  });
  it("converges concurrent execution attempts on one provider submission", async () => {
    const { withdrawal } = await setup();
    const results = await Promise.all([
      app.payoutExecution.execute(withdrawal.id, newId()),
      app.payoutExecution.execute(withdrawal.id, newId()),
    ]);
