...
export class JsonConsoleLogger implements WorkerLogger {
  info(fields: Record<string, unknown>, message: string) {
    console.log(
      JSON.stringify({ level: "info", message, ...fields, timestamp: new Date().toISOString() }),
    );
  }
...
export class OutboxHandlerRegistry {
  ...
  register(handler: OutboxEventHandler): this {
    for (const name of handler.eventNames) {
      if (this.handlers.has(name)) throw new Error(`Outbox handler already registered: ${name}`);
      this.handlers.set(name, handler);
    }
    return this;
  }
  get(eventName: string): OutboxEventHandler {
    const handler = this.handlers.get(eventName);
    if (!handler) throw new Error(`No outbox handler registered: ${eventName}`);
    return handler;
  }
}
...
const defaults: OutboxDispatcherOptions = {
  batchSize: 20,
...
export class OutboxDispatcher {
  ...
  constructor(
    ...
    private readonly logger: WorkerLogger = new JsonConsoleLogger(),
  ...
  async run(signal: AbortSignal): Promise<void> {
    ...
    while (!signal.aborted) {
      try {
        ...
        if (processed === 0) await wait(this.options.pollMilliseconds, signal);
      } catch (error) {
        this.logger.error(
          { worker_id: this.workerId, error: errorMessage(error) },
          "outbox.poll.failed",
        );
  ...
  private async dispatch(event: ClaimedOutboxEvent): Promise<void> {
    ...
    try {
      await this.handlers.get(event.name).handle(event);
