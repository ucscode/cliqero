...
import { PurchaseCompletionProcessor } from "@/processors/purchase/completion";
...
describe("PurchaseCompletionProcessor", () => {
  it("creates one purchase and entitlement when completion is retried", async () => {
    ...
    const processor = new PurchaseCompletionProcessor(
      {
        findById: async (id) =>
          [...purchases.values()].find((purchase) => purchase.id === id) ?? null,
        findByIdempotencyKey: async (key) => purchases.get(key) ?? null,
        save: async (purchase) => {
          purchases.set(purchase.idempotencyKey, purchase);
        },
      },
      {
        findByPurchaseId: async (purchaseId) =>
          [...entitlements.values()].find((item) => item.purchaseId === purchaseId) ?? null,
