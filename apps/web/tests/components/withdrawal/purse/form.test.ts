...
const form = readFileSync(
  resolve(process.cwd(), "src/components/withdrawal/purse/form.tsx"),
  "utf8",
);
const page = readFileSync(
  resolve(process.cwd(), "src/components/withdrawal/purse/form-page.tsx"),
  "utf8",
);

describe("purse form pages", () => {
  it("does not preselect a method and gates details and save behind an explicit choice", () => {
    ...
    expect(form).toContain("Choose a method");
    expect(form).toContain("{method && (");
    expect(form).toContain('"Save purse"');
  ...
  it("reuses one form for add and edit, with immutable method on edit and persisted API calls", () => {
    expect(page).toContain("<PurseForm");
  ...
  it("wires dedicated create and dynamic edit routes into the existing dashboard shell", () => {
    const createRoute = readFileSync(
      resolve(process.cwd(), "src/app/dashboard/purse/new/page.tsx"),
      "utf8",
    );
    const editRoute = readFileSync(
      resolve(process.cwd(), "src/app/dashboard/purse/[destinationId]/edit/page.tsx"),
      "utf8",
    );
    ...
    expect(createRoute).toContain('purseFormMode="create"');
    expect(editRoute).toContain('purseFormMode="edit"');
    expect(editRoute).toContain("params: Promise<{ destinationId: string }>");
    expect(shell).toContain("<PurseFormPage mode={purseFormMode}");
    expect(page).toContain('const purseListHref = "/dashboard?section=purse"');
  });
});
