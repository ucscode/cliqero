...
describe("country currency resolution", () => {
  ...
  it("loads the canonical mapping from repository and app working directories", () => {
    const originalDirectory = process.cwd();
    const appDirectory = originalDirectory;
    const repositoryDirectory = resolve(appDirectory, "../..");
    try {
      for (const directory of [repositoryDirectory, appDirectory]) {
