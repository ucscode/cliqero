...
import { AccessGrant, AccessService } from "@/modules/access/access";
...
describe("AccessService", () => {
  ...
  const recordingGrants = () => {
    const saved: AccessGrant[] = [];
    return {
      saved,
      repository: {
        findByTokenHash: async () => null,
        save: async (grant: AccessGrant) => {
          saved.push(grant);
        },
      },
    };
  ...
  it("issues an opaque credential and verifies it only for an independently authorized integration", async () => {
    ...
    const service = new AccessService(
      {
        findByPurchaseId: async () => entitlement,
        findActive: async (buyerId, listingId) =>
          buyerId === entitlement.buyerId && listingId === entitlement.listingId
            ? entitlement
            : null,
      ...
      },
      {
        ...
        save: async (grant) => {
          grants.set(grant.tokenHash.toString("hex"), grant);
        },
      },
    );
    ...
    await expect(
      service.verify(source, { id: "integration-1", canVerifyListing: () => false }),
    ).resolves.toEqual({ authorized: false });
    await expect(
      service.verify(source, { id: "integration-1", canVerifyListing: (id) => id === "listing-1" }),
  ...
  it.each([
    ...
    const grants = recordingGrants();
    const service = new AccessService(repositoryFor(entitlement), grants.repository);
    await expect(service.issue("buyer-1", "listing-1")).resolves.toHaveProperty("source");
    expect(grants.saved).toHaveLength(1);
  });
  ...
  it.each([
    ...
    [
      "revoked",
      (() => {
        const value = new Entitlement("entitlement-1", "buyer-1", "listing-1", "purchase-1");
        value.revoke();
        return value;
      })(),
    ],
  ])("does not issue an access grant for an entitlement with %s", async (_label, entitlement) => {
    const grants = recordingGrants();
    const service = new AccessService(repositoryFor(entitlement), grants.repository);
    await expect(service.issue("buyer-1", "listing-1")).rejects.toThrow(
      "Active entitlement not found",
    );
    expect(grants.saved).toHaveLength(0);
  });
});
